    ...
    try:
        ...
        if date_filter and date_filter != "all":
            if date_filter == "today":
                where_clause += " AND DATE(timestamp) = CURRENT_DATE"
            elif date_filter == "week":
                where_clause += " AND timestamp >= DATE_TRUNC('week', CURRENT_DATE)"
            elif date_filter == "month":
                where_clause += " AND timestamp >= DATE_TRUNC('month', CURRENT_DATE)"
    ...
    try:
        ...
        if existing:
            ...
            if update_fields:
                ...
                row = await conn.fetchrow(
                    "SELECT * FROM alert_settings WHERE id = $1", existing["id"]
                )
        else:
